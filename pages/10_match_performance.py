...
def deserialize_data(stored_data):
    ...
    try:
        ...
        # Example: Convert potential datetime strings back if needed
        # (Assuming no dates are critical for THIS page based on original app8 code)
...
# --- REMOVE Data Loading and Preprocessing ---
# The following code block should be DELETED from this file.
# It will be moved/integrated into multi_page_app.py's load_tournament_data function.
...
# --- Map Cities/Venues to Countries ---
# !! IMPORTANT !! This dictionary needs to be in multi_page_app.py's load_tournament_data
# city_to_country = { ... }

# --- Pre-calculate match teams ---
# This logic needs to be in multi_page_app.py's load_tournament_data
# print("Calculating unique teams per match...")
# match_teams = df.groupby(...)
# df = pd.merge(...)
# ...
...
# This function and its application need to be in multi_page_app.py's load_tournament_data
# def get_bowling_team_revised(row):
#     # ...
# print("Deriving 'bowling_team' column...")
# df['bowling_team'] = df.apply(...)
# print("Derived 'bowling_team' column.")
# --- END OF DELETED BLOCK ---

# --- REMOVE Get unique values for dropdowns ---
# This will be done inside the callback now
# --- START OF DELETED BLOCK ---
# if not df.empty and 'event_name' in df.columns:
#     unique_events = df['event_name'].dropna().unique()
#     unique_events.sort()
#     print(f"Found {len(unique_events)} unique events with valid matches.")
# else:
#     print("Warning: Could not extract unique events.")
#     unique_events = []
# --- END OF DELETED BLOCK ---
...
# --- App Layout ---
# CHANGE: Assign to 'layout' instead of 'app.layout'
layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(html.H4("Hierarchical Breakdowns", className="text-center"), width=12)
    ], id='me-stats-sunburst-header', style={'display': 'none'}), # New Header Row
    # Combined Sunburst Row
    dbc.Row([
        dbc.Col(dcc.Loading(dcc.Graph(id='me-dismissal-sunburst-chart')), md=6), # Added Loading
        dbc.Col(dcc.Loading(dcc.Graph(id='me-extras-sunburst-chart'))    , md=6)  # Added Loading
...
def update_world_map(filtered_match_ids, team1, team2, stored_tournament_data):
    ...
    map_df_unique_matches = map_df_unique_matches.dropna(subset=['country']) # Drop matches with no country mapping
...
def update_stadium_dropdown(clickData, filtered_match_ids, stored_tournament_data):
    ...
    if not unique_venues.any():
        return html.Div(f"No specific stadiums found in {clicked_country} for these matches.", className="text-warning"), country_filtered_df['match_id'].unique().tolist() # Return IDs even if no named venue


    venue_options = [{'label': venue, 'value': venue} for venue in unique_venues]
    country_match_ids = country_filtered_df['match_id'].unique().tolist() # Match IDs for this country
    ...
    output_div = html.Div([
        ...
        dcc.Dropdown(id='me-venue-dropdown', options=venue_options, placeholder="Select a stadium...", clearable=False, searchable=True, value=None) # Use prefixed ID
    ])
...
def update_stats(venue_match_ids, team1, team2, stored_tournament_data):
    ...
    visible_style = {'display': 'flex'} # Or 'block' depending on desired layout flow
    ...
    if df_tournament is None or df_tournament.empty:
        ...
        figures = {k: err_fig for k in figures} # Set all figures to error figure
    ...
    try: # Stat 2: Toss Analysis
        ...
        if toss_data['Toss Wins'].sum() > 0: # Only plot if there were tosses involving these teams
    ...
    try: # Stat 4: Runs Scored Comparison
        ...
        if 'runs_off_bat' in stats_df.columns and 'batting_team' in stats_df.columns:
            ...
            if not runs_data.empty and runs_data['runs_off_bat'].sum() > 0 :
                ...
                runs_data['Plot_Value'] = runs_data['runs_off_bat'].replace(0, tiny_value) # Offset zero for plotting
                figures['runs'] = px.bar(runs_data, x='batting_team', y='Plot_Value', color='batting_team', title=f"Total Runs Scored (Off Bat) at {venue_name}", labels={'batting_team': 'Team', 'Plot_Value': 'Runs Scored'}, color_discrete_sequence=px.colors.qualitative.Safe, text='runs_off_bat') # Show original value as text
                yaxis_max_runs = max(original_runs_max * 1.1, tiny_value * 2) # Adjust y-axis range
    ...
    try: # Stat 5: Wickets Lost Comparison
        wickets_col = 'player_dismissed' # Assuming 1 if dismissed, 0 otherwise - check preprocessing
    ...
    try:
        ...
        if 'out_kind' in stats_df.columns and 'batting_team' in stats_df.columns:
            ...
            dismissal_df = stats_df[~stats_df['out_kind'].isin(['', ' ', 'not out', 'retired hurt', 'obstructing the field', 'Unknown', np.nan])].copy() # Add more if needed
            dismissal_df['out_kind'] = dismissal_df['out_kind'].fillna('Unknown').str.lower().str.strip()

            dismissals_by_team = dismissal_df[dismissal_df['batting_team'].isin([team1, team2])].groupby(['batting_team', 'out_kind']).size().reset_index(name='Count')
            ...
            if dismissal_data_original is not None and not dismissal_data_original.empty:
                 dismissal_data_original['Dismissal Type'] = dismissal_data_original['out_kind'].str.replace('_', ' ').str.title()
                 figures['sunburst'] = px.sunburst(dismissal_data_original, path=['batting_team', 'Dismissal Type'], values='Count', title=f"Dismissals Breakdown at {venue_name}", color='batting_team', color_discrete_sequence=px.colors.qualitative.Pastel)
                 figures['sunburst'].update_layout(margin=dict(t=50, l=10, r=10, b=10))
            else: figures['sunburst'] = go.Figure(layout=go.Layout(title=f"No Dismissal Data at {venue_name}"))
        else: figures['sunburst'] = go.Figure(layout=go.Layout(title=f"Dismissal ('out_kind') Data Missing"))
    ...
    try: # Stat 8: Runs vs Wickets Scatter Plot
        if 'runs_off_bat' in stats_df.columns and wickets_col in stats_df.columns and 'batting_team' in stats_df.columns and 'match_id' in stats_df.columns:
            ...
            match_summary = pd.merge(match_summary, stats_df.drop_duplicates(subset=['match_id'])[['match_id', 'winner']], on='match_id', how='left') # Merge winner info
            match_summary = match_summary[match_summary['batting_team'].isin([team1, team2])]
            match_summary['winner'] = match_summary['winner'].fillna('Other (Tie/NR)') # Handle non-wins
...
@callback(
    ...
    Output('me-dismissal-sunburst-chart', 'figure', allow_duplicate=True), Output('me-extras-sunburst-chart', 'figure', allow_duplicate=True),
