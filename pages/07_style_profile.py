...
def layout():
    return dbc.Container(fluid=True, children=[
        ...
        dbc.Row(dbc.Col(html.H2("Recent Form Comparison", className="text-center my-4"), width=12)),

        # Controls: Player1, Player2, Format, Last N Innings
        dbc.Row([
            dbc.Col([
                html.Label("Player 1:", className="fw-bold"),
                dcc.Dropdown(id='form-player-1', placeholder="Select player 1"),
            ], width=3),
            dbc.Col([
                html.Label("Player 2:", className="fw-bold"),
                dcc.Dropdown(id='form-player-2', placeholder="Select player 2"),
            ], width=3),
            dbc.Col([
                html.Label("Format:", className="fw-bold"),
                dbc.RadioItems(
                    id='form-format',
                    options=[{'label': f, 'value': f} for f in ["ODI","T20","T20I","Test"]],
                    value="ODI", inline=True
                )
            ], width=3),
            dbc.Col([
                html.Label("Last N Innings:", className="fw-bold"),
                dbc.Input(id='form-n', type='number', min=1, max=50, step=1, value=5)
            ], width=3),
        ], className="mb-4"),

        # Shared line chart
        dbc.Row(dbc.Col(dcc.Loading(dcc.Graph(id='form-line')), width=12), className="mb-4"),
...
@callback(
    ...
    Input('form-player-1','value'),
    Input('form-player-2','value'),
    Input('form-format',  'value'),
    Input('form-n',       'value'),
    State('main-data-store','data'),
    prevent_initial_call=True
)
def update_form(p1, p2, fmt, n, store_data):
    df = _load_df(store_data)
    # placeholder if inputs missing
    empty = go.Figure(); empty.update_layout(title="Select both players, format & N")
    ...
    def get_player_data(player):
        ...
        career_avg = d['runs_scored'].sum() / total_outs if total_outs>0 else 0
        # form average
        form_outs = d_recent['player_out'].sum()
        form_avg = d_recent['runs_scored'].sum() / form_outs if form_outs>0 else 0
        # dismissal pie
        outs = d_recent[d_recent['player_out']==1]
        if not outs.empty:
            pie_df = outs['out_kind'].fillna('Unknown').value_counts().reset_index()
    ...
    def make_gauge(player, form_avg, career_avg):
        return go.Figure(go.Indicator(
            ...
            value=form_avg,
            delta={'reference':career_avg,'valueformat':'.1f','position':'top'},
