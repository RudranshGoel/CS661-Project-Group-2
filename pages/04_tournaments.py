...
def layout():
    ...
    return dbc.Container([
        ...
        dbc.Row([
            dbc.Col([
                html.Label("Select Tournament:", htmlFor='tournament-dropdown'),
                dcc.Dropdown(id='tournament-dropdown',
                             options=[{'label': 'Loading tournaments...', 'value': '', 'disabled': True}],
                             value=None,
                             clearable=False,
        ...
        dbc.Row([
            dbc.Col(
                dcc.Loading(
                    ...
                    children=[dcc.Graph(id='tournament-wins-graph', figure=go.Figure())]
                ), width=12
            )
        ]),
        dbc.Row([
            dbc.Col(html.Div(id='tournament-info-message', style={'marginTop': '15px'}), width=12)
        ]),
        dbc.Row([
            dbc.Col(
                 dcc.Loading(
                    ...
                    children=[
                        html.Div(id='runs-boxplot-title', style={'textAlign': 'center', 'marginTop': '20px', 'fontWeight': 'bold'}),
                        dcc.Graph(id='team-season-runs-boxplot', figure=go.Figure())
                    ]
                ),
                width=12, md=6,
                className="mt-4"
             ...
             dbc.Col(
                 dcc.Loading(
                    ...
                    children=[
                        html.Div(id='dismissals-boxplot-title', style={'textAlign': 'center', 'marginTop': '20px', 'fontWeight': 'bold'}),
                        dcc.Graph(id='team-season-dismissals-boxplot', figure=go.Figure())
                    ]
                ),
                width=12, md=6,
                className="mt-4"
...
def update_tournament_dropdown(tournament_data_json):
    ...
    try:
        ...
        options = [{'label': name, 'value': name} for name in sorted_tournaments]
...
def update_tournament_wins_graph(selected_tournament, tournament_data_json):
     # ... (Code identical to previous working version) ...
    print(f"Callback: update_tournament_wins_graph triggered for '{selected_tournament}'")
    info_message = ""
    ...
    try:
        ...
        if df_tournaments.empty:
             print("  - Tournament DataFrame is empty after deserialization for histogram.")
        ...
        if tournament_df.empty:
            ...
            info_message = dbc.Alert(f"No match winner data found for '{selected_tournament}'.", color="info")
        ...
        print(f"  - Found {len(win_counts)} teams with wins for '{selected_tournament}'.")
        fig = px.bar(
        ...
        fig.update_layout(
            xaxis_title="Team", yaxis_title="Number of Matches Won",
...
def update_team_season_boxplots(clickData, selected_tournament, tournament_data_json):
    ...
    # Basic checks
    if clickData is None: return empty_return
    if not selected_tournament: return empty_return
    if tournament_data_json is None:
        error_msg = dbc.Alert("Error: Tournament data is not available.", color="danger")
        return go.Figure(), error_msg, go.Figure(), error_msg

    try:
        # Extract clicked team
        try: clicked_team = clickData['points'][0]['x']
        except (KeyError, IndexError, TypeError): return empty_return
        print(f"  - Clicked team: '{clicked_team}'")

        # Load data
        df_tournaments = pd.read_json(tournament_data_json, orient='split')
        if df_tournaments.empty:
            error_msg = dbc.Alert("Error: Tournament data is empty.", color="warning")
            return go.Figure(), error_msg, go.Figure(), error_msg
        ...
        dismissals_req_cols = {'season', 'player_dismissed', 'match_id', 'batting_team'}

        # --- Check for Batting Team Column ---
        team_batted_col = 'batting_team'
        if team_batted_col not in df_tournaments.columns:
            print(f"  - CRITICAL ERROR: Column '{team_batted_col}' is required and missing.")
            error_msg = dbc.Alert(f"Error: Required column '{team_batted_col}' missing.", color="danger")
            # Attempt to show runs plot if possible, error for dismissals
            fig_runs, runs_title = generate_runs_plot(df_tournaments, selected_tournament, clicked_team, runs_req_cols, team_batted_col) # Use a helper potentially, or inline
            return fig_runs, runs_title, go.Figure(), error_msg

        # --- Filter Data for Selected Team (Batting) & Tournament ---
        print(f"  - Filtering for event_name='{selected_tournament}', {team_batted_col}='{clicked_team}'")
        team_df_filtered = df_tournaments[
            (df_tournaments['event_name'] == selected_tournament) &
            (df_tournaments[team_batted_col] == clicked_team)
        ].copy()
        print(f"  - Records after filtering for team batting & tournament: {len(team_df_filtered)}")

        if team_df_filtered.empty:
            info_msg = dbc.Alert(f"No data found where '{clicked_team}' batted in '{selected_tournament}'.", color="info")
        ...
        if runs_req_cols.issubset(df_tournaments.columns):
            ...
            runs_df['runs_off_bat'] = pd.to_numeric(runs_df['runs_off_bat'], errors='coerce')
            ...
            if not runs_df.empty:
                ...
                print(f"  - Found {len(runs_df)} valid records for runs boxplot.")
                ...
                runs_title = dbc.Alert(f"No valid run data found for '{clicked_team}' (batting).", color="info")
        else:
            missing_runs_cols = runs_req_cols - set(df_tournaments.columns)
            print(f"  - ERROR: Missing columns for runs plot: {missing_runs_cols}.")
            runs_title = dbc.Alert(f"Error: Missing data columns ({', '.join(missing_runs_cols)}) for runs plot.", color="danger")


        # --- Generate Dismissals Box Plot (using numeric 'player_dismissed') ---
        if dismissals_req_cols.issubset(df_tournaments.columns):
            print("  - Preparing Dismissals Box Plot (using numeric 'player_dismissed')...")
            ...
            dismissals_df['dismissal_count'] = pd.to_numeric(dismissals_df['player_dismissed'], errors='coerce')

            # --- DEBUG ---
            print(f"  - Sample 'dismissal_count' after conversion: {dismissals_df['dismissal_count'].dropna().unique()[:10]}")
            print(f"  - Stats for converted dismissal_count:\n{dismissals_df['dismissal_count'].describe()}")

            # 2. Drop rows where conversion failed or season is missing
            dismissals_df.dropna(subset=['season', 'dismissal_count'], inplace=True)
            ...
            print("  - Assuming data is event-level. Finding max dismissal count per match...")
            dismissal_summary_per_match = dismissals_df.loc[
                dismissals_df.groupby(['season', 'match_id'])['dismissal_count'].idxmax()
            ]
            ...
            print(f"  - Final data for dismissals plot (sample):\n{dismissal_summary_per_match[['season', 'match_id', 'dismissal_count']].head()}")


            # 5. Plot the summary data
            if not dismissal_summary_per_match.empty:
                dismissal_summary_per_match = dismissal_summary_per_match.sort_values(by='season')
                print(f"  - Found {len(dismissal_summary_per_match)} matches with dismissal data for boxplot.")

                fig_dismissals = px.box(
                    dismissal_summary_per_match,
                    x='season',
                    y='dismissal_count', # Use the numeric column directly (after potential aggregation)
                    points="all",
                    labels={'season': 'Season', 'dismissal_count': 'Dismissals Recorded'}
                )
                ...
                dismissals_title = dbc.Alert(f"No valid dismissal data found for '{clicked_team}' (batting).", color="info")

        elif not dismissals_req_cols.issubset(df_tournaments.columns):
            missing_dismissal_cols = dismissals_req_cols - set(df_tournaments.columns)
            print(f"  - ERROR: Missing columns for dismissals plot: {missing_dismissal_cols}.")
            dismissals_title = dbc.Alert(f"Error: Missing data columns ({', '.join(missing_dismissal_cols)}) for dismissals plot.", color="danger")
        ...
        print(f"  - !!! UNEXPECTED ERROR in update_team_season_boxplots: {e} !!!")
        print(traceback.format_exc())
        error_msg = dbc.Alert(f"An unexpected error occurred generating box plots: {e}", color="danger")
        return go.Figure(), error_msg, go.Figure(), error_msg
