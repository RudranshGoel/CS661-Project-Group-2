...
def get_country_code(name, code_type='alpha_3'):
    ...
    overrides_alpha3 = { "United Arab Emirates": "ARE", "Scotland": "GBR", "United States of America": "USA", "Netherlands": "NLD", "England": "GBR", "Ireland": "IRL", "West Indies": "JAM", # Use Jamaica as a representative ISO for map, actual data uses 'West Indies'
                         "Hong Kong": "HKG", "Papua New Guinea": "PNG", "Bermuda": "BMU", "Afghanistan": "AFG", "Bangladesh": "BGD", "India": "IND", "Pakistan": "PAK", "Sri Lanka": "LKA", "Australia": "AUS", "New Zealand": "NZL", "South Africa": "ZAF", "Zimbabwe": "ZWE", "Kenya": "KEN", "Canada": "CAN", "Namibia": "NAM", "Nepal": "NPL", "Oman": "OMN", "ICC World XI": None, # No specific country
                          "Asia XI": None, "Africa XI": None, "East Africa": None, # Historically relevant but no single modern ISO
                        }
    ...
    name_map = {
        ...
        'England Lions': None, 'Ireland A': None, 'South Africa A': None, # Exclude A teams explicitly
    }
    ...
    try:
        ...
        try:
            ...
            if 'england' in std_name.lower(): return "GBR"
            if 'scotland' in std_name.lower(): return "GBR" # Map Scotland to GBR for plotting simplicity if needed
            if 'ireland' in std_name.lower(): return "IRL" # Ensure Ireland maps correctly

            results = pycountry.countries.search_fuzzy(std_name)
            if results:
                return results[0].alpha_3 if code_type == 'alpha_3' else results[0].alpha_2
            else:
                 print(f"Warning: Could not find ISO code for '{name}' (standardized: '{std_name}')")
...
def get_all_country_iso3():
    ...
    override_codes = { "ARE", "GBR", "USA", "NLD", "IRL", "JAM", "HKG", "PNG", "BMU", "AFG", "BGD", "IND", "PAK", "LKA", "AUS", "NZL", "ZAF", "ZWE", "KEN", "CAN", "NAM", "NPL", "OMN"}
    codes.update(override_codes)
    return codes

def export_pdf(stats_text, filename="player_stats.pdf"):
    # (Keep the function definition)
    buffer = BytesIO(); c = canvas.Canvas(buffer, pagesize=letter); width, height = letter; text = c.beginText(inch, height - inch); text.setFont("Helvetica", 10)
    for line in stats_text.split("\n"): text.textLine(line)
    c.drawText(text); c.save(); buffer.seek(0); return buffer

def plot_dismissal_pie(df, title="Dismissal Analysis"):
    # (Keep the function definition - check columns exist etc.)
    if df is None or df.empty or not all(col in df.columns for col in ['Count', 'Dismissal Type']): return go.Figure(layout=go.Layout(title=go.layout.Title(text=f"{title}<br>(Data missing)")))
    if df['Count'].sum() == 0: return go.Figure(layout=go.Layout(title=go.layout.Title(text=f"{title}<br>(No dismissals)")))
    try:
        fig = px.pie(df, names='Dismissal Type', values='Count', title=title, hole=0.35, color_discrete_sequence=px.colors.qualitative.Pastel)
        fig.update_traces(textposition='inside', textinfo='percent+label', pull=[0.05]*len(df)); fig.update_layout(showlegend=False, margin=dict(t=50, b=0, l=0, r=0)); return fig
    except Exception as e: print(f"Error creating dismissal pie: {e}"); return go.Figure(layout=go.Layout(title=go.layout.Title(text=f"{title}<br>(Plotting error)")))


def plot_run_contribution_pie(runs_4s, runs_6s, other_runs, title="Run Scoring Breakdown"):
    # (Keep the function definition)
    data = {'Run Type': ['Runs from 4s', 'Runs from 6s', 'Other Runs (1s, 2s, 3s)'], 'Runs': [runs_4s, runs_6s, other_runs]}; df_runs = pd.DataFrame(data); df_runs = df_runs[df_runs['Runs'] > 0]
    if df_runs.empty: return go.Figure(layout=go.Layout(title=go.layout.Title(text=f"{title}<br>(No runs scored)")))
    try:
        fig = px.pie(df_runs, names='Run Type', values='Runs', title=title, hole=0.35, color_discrete_sequence=px.colors.qualitative.Set2)
        fig.update_traces(textposition='outside', textinfo='percent+value', pull=[0.05]*len(df_runs)); fig.update_layout(showlegend=False, margin=dict(t=50, b=0, l=0, r=0)); return fig
    except Exception as e: print(f"Error creating run contribution pie: {e}"); return go.Figure(layout=go.Layout(title=go.layout.Title(text=f"{title}<br>(Plotting error)")))
...
def create_comparison_gauge(value, reference, title, lower_bound=0, upper_bound=None, higher_is_better=True):
    ...
    value = float(value) if value is not None and not np.isnan(value) else np.nan # Handle NaN explicitly
    reference = float(reference) if reference is not None and not np.isnan(reference) else np.nan # Handle NaN explicitly
    ...
    ref_calc = reference if pd.notna(reference) and np.isfinite(reference) else np.nan
    ...
    valid_values = [v for v in [value_calc, ref_calc] if pd.notna(v) and np.isfinite(v)]
    max_val = max(valid_values) if valid_values else 0 # Default to 0 if no valid values


    if upper_bound is None:
        # Set upper bound slightly above the max of value or reference
        upper_bound = max_val * 1.5 if max_val > 0 else (lower_bound + 10) # Add buffer, handle zero case
    else:
    ...
    delta_config = {'reference': ref_calc, 'relative': False, 'valueformat': '.2f'} if pd.notna(ref_calc) else None

    try:
        fig = go.Figure(go.Indicator(
            mode="gauge+number" + ("+delta" if delta_config else ""), # Conditionally add delta
        ...
        fig.update_layout(height=250, margin=dict(t=60, b=10, l=30, r=30)); return fig
    except Exception as e:
...
def create_comparison_bar(value_opp, value_career, label_opp, label_career, title):
    ...
    value_opp = float(value_opp) if pd.notna(value_opp) and np.isfinite(value_opp) else 0.0
    value_career = float(value_career) if pd.notna(value_career) and np.isfinite(value_career) else 0.0

    def format_value(v):
        if isinstance(v, (int, float)):
             # Format floats with precision, integers without decimals
             if isinstance(v, float): return f'{v:,.2f}'
             else: return f'{v:,}' # Integer formatting
        return str(v) # Fallback for other types

    labels = [label_opp, label_career]; values = [value_opp, value_career]; colors = ['#1f77b4', '#ff7f0e']
    try:
        fig = go.Figure(data=[go.Bar(x=labels, y=values, text=[format_value(v) for v in values], textposition='auto', marker_color=colors)])
        fig.update_layout(title=title, xaxis_title="Context", yaxis_title="Value", height=300, margin=dict(t=50, b=30, l=30, r=30), yaxis=dict(rangemode='tozero')); return fig
    except Exception as e: print(f"Error creating bar chart '{title}': {e}"); return go.Figure(layout=go.Layout(title=go.layout.Title(text=f"{title}<br>(Plotting error)"), height=300))
...
COLOR_PLAYED_OUTSIDE_RANGE = "#FFA07A" # Light Salmon for played but outside selected date range (optional, can simplify)

# --- Register Page ---
dash.register_page(__name__, name='Player Analyzer', path='/analyzer', title='Player Analyzer') # Add title
...
def layout():
    ...
    initial_start_date = today - datetime.timedelta(days=5*365) # Default to 5 years ago

    return dbc.Container([
        dcc.Store(id='analyzer-iso-map-store', storage_type='memory', data={}),
        dcc.Store(id='analyzer-selected-opposition-store', storage_type='memory', data=None),
        ...
        dbc.Row([
            ...
            dbc.Col([
                dbc.Card([
                     ...
                     dbc.CardBody([
                         html.Div([
                            html.Label("1. Select Player:", className="fw-bold"),
                            dcc.Dropdown( id='analyzer-player-dropdown', options=initial_player_opts, value=None, placeholder="Select player...", clearable=False),
                         ], className="mb-3"),
                         html.Div([
                            html.Label("2. Select Format:", className="fw-bold"),
                            dbc.RadioItems(id='analyzer-format-radio', options=[{'label': fmt, 'value': fmt} for fmt in ALLOWED_FORMATS], value=ALLOWED_FORMATS[0], inline=True, labelStyle={'margin-right': '15px'}, inputClassName="me-1"),
                         ], className="mb-3"),
                         # --- ADDED DATE PICKER ---
                         html.Div([
                             html.Label("3. Select Date Range:", className="fw-bold"),
                             dcc.Loading( # Add loading indicator around date picker
                                 ...
                                 children=[
                                     dcc.DatePickerRange(
                                        ...
                                        min_date_allowed=initial_start_date - datetime.timedelta(days=20*365), # Default wide range
                         ...
                         # --- END OF DATE PICKER ---
                         html.Hr(),
                         html.P(["4. Click a country on the map to see stats against that opponent within the selected period."], className="text-muted fst-italic"),
                         html.P([
                             html.Span("■", style={'color': COLOR_PLAYED, 'fontSize': '20px'}), " Opponent Played (in period)", html.Br(),
                             html.Span("■", style={'color': COLOR_NOT_PLAYED, 'fontSize': '20px'}), " Opponent Not Played (in period)"
                ...
                dbc.Card([
                     ...
                     dbc.CardBody(
                         dcc.Loading(
                             ...
                             children=[ dcc.Graph( id='analyzer-map-graph', config={'displayModeBar': False}, style={'height': '450px'} )]
                         )
                    )
                ])
            ], width=12, lg=8),
        ...
        html.Div(id='analyzer-stats-output-area', children=default_placeholder) # Start with placeholder
            ...
            try:
                 ...
                 if not pd.api.types.is_datetime64_any_dtype(df['start_date']):
                     ...
                     if df['start_date'].isnull().any():
                          print("Warning: Some 'start_date' values failed conversion during deserialization.")
                 # Optional: Convert to date if time part is irrelevant and consistently midnight
                 # df['start_date'] = df['start_date'].dt.date
            except Exception as e:
                 print(f"Warning: Could not convert 'start_date' back to datetime after deserialization: {e}")
                 # Decide if failure is critical - maybe drop rows or return None?
                 df.drop(columns=['start_date'], inplace=True, errors='ignore') # Or handle differently
        else:
...
def update_analyzer_date_picker_range(selected_player, selected_format, stored_data):
    print(f"Callback triggered: update_analyzer_date_picker_range (Player: {selected_player}, Format: {selected_format})")
    if not selected_player or not selected_format:
        print("  - Player or format not selected. Disabling date picker.")
        today = datetime.date.today()
        min_allowed = today - datetime.timedelta(days=30*365) # Default past range
    ...
    if df is None or df.empty or 'start_date' not in df.columns or df['start_date'].isnull().all():
        print("  - Data unavailable or 'start_date' missing/invalid. Disabling date picker.")
        today = datetime.date.today()
    ...
    if player_format_df.empty:
        print(f"  - No data found for {selected_player} in {selected_format}. Disabling date picker.")
        today = datetime.date.today()
        min_allowed = today - datetime.timedelta(days=30*365)
        return min_allowed, today, None, None, True # Disable picker
...
@callback(
    ...
    Output('analyzer-selected-opposition-store', 'data', allow_duplicate=True), # Reset opposition
    Input('analyzer-player-dropdown', 'value'),
    Input('analyzer-format-radio', 'value'),
...
def analyzer_update_map(selected_player, selected_format, start_date, end_date, stored_data):
    ...
    print(f"Callback triggered: analyzer_update_map (Player: {selected_player}, Format: {selected_format}, Range: {start_date} to {end_date})")
    ...
    try:
        ...
        start_dt = pd.to_datetime(start_date).normalize() # Normalize to midnight for date comparison
    ...
    if player_df.empty:
        print(f"  - No {selected_format} data found for {selected_player} within {start_date} to {end_date}.")
        ...
        world_df['HoverName'] = world_df['iso_code'].apply(lambda x: f"Unknown ({x})") # Basic hover
        world_df['TotalRuns'] = 0
        world_df['Innings'] = 0
        fig_map_empty = px.choropleth( world_df, locations="iso_code", locationmode="ISO-3", color_discrete_map={True: COLOR_PLAYED, False: COLOR_NOT_PLAYED}, color="PlayedAgainst", hover_name="HoverName", hover_data={"iso_code": False, "PlayedAgainst": False, "TotalRuns": False, "Innings": False}, custom_data=['iso_code'])
        fig_map_empty.update_layout( title=f"No opponents played by {selected_player} ({selected_format}) between {start_date} and {end_date}", title_x=0.5, showlegend=False, geo=dict(showframe=False, showcoastlines=False, projection_type='natural earth', bgcolor='rgba(0,0,0,0)', landcolor=COLOR_NOT_PLAYED), margin={"r":0,"t":40,"l":0,"b":0})
    ...
    if not valid_opps.empty:
         iso_to_opposition_map = valid_opps.groupby('iso_code')['opposition_team'].first().to_dict()
         print(f"  - Opponents played in period (ISO map): {iso_to_opposition_map}")
    ...
    world_df['PlayedAgainst'] = world_df['iso_code'].isin(iso_to_opposition_map.keys()) # Played if in our map
    world_df['HoverName'] = world_df['iso_code'].apply(
        lambda code: iso_to_opposition_map.get(code, f"Not Played ({code})") if pd.notna(code) else "N/A"
    )
    ...
    try:
        ...
        fig_map.update_layout(
            ...
            geo=dict(
                ...
                subunitcolor='rgba(255,255,255,0.5)' # Light borders between subunits
            ),
...
@callback(
    ...
    State('analyzer-iso-map-store', 'data'), # Use the map generated by the previous callback
    prevent_initial_call=True
)
def analyzer_update_selected_opposition(clickData, iso_map_data):
    ...
    try:
        ...
        elif 'location' in point: # Fallback if customdata isn't there (should be)
        ...
        print(f"Callback triggered: analyzer_update_selected_opposition (Clicked ISO: {clicked_iso})")
        ...
        if clicked_iso in iso_map_data:
            ...
            print(f"  - Resolved opposition (played in period): {opposition_name}")
            ...
            print(f"  - Clicked ISO '{clicked_iso}' not found in playable opponents for the period.")
...
@callback(
    ...
    Input('analyzer-selected-opposition-store', 'data'), # Triggered by map click (or reset)
...
def analyzer_update_stats_area(selected_opposition, selected_player, selected_format, start_date, end_date, stored_data):
    ...
    print(f"Callback triggered: analyzer_update_stats_area (Opposition: {selected_opposition}, Period: {start_date} to {end_date})")
    ...
    try:
        ...
        career_stats_df_period = df_full[career_mask].copy() # Use this for comparison gauges/bars
    ...
    try:
        ...
        opp_matches = country_stats_df['match_id'].nunique() # Use match_id for unique matches
        ...
        opp_bat_sr = (opp_total_runs / opp_balls_faced) * 100 if opp_balls_faced > 0 else 0.0
        ...
        opp_bowl_econ = (opp_runs_conceded / opp_balls_bowled) * 6 if opp_balls_bowled > 0 else np.nan # Handle division by zero for econ

        # Career Stats (in period - for comparison)
        car_period_matches = career_stats_df_period['match_id'].nunique()
        car_period_total_runs = int(career_stats_df_period["runs_scored"].sum())
        car_period_balls_faced = int(career_stats_df_period["balls_faced"].sum())
        car_period_outs = int(career_stats_df_period["player_out"].sum())
        car_period_bat_avg = car_period_total_runs / car_period_outs if car_period_outs > 0 else np.nan
        car_period_bat_sr = (car_period_total_runs / car_period_balls_faced) * 100 if car_period_balls_faced > 0 else 0.0
        car_period_wickets = int(career_stats_df_period["wickets_taken"].sum())
        car_period_balls_bowled = int(career_stats_df_period["balls_bowled"].sum())
        car_period_runs_conceded = int(career_stats_df_period["runs_conceded"].sum())
        car_period_bowl_avg = car_period_runs_conceded / car_period_wickets if car_period_wickets > 0 else np.nan
        car_period_bowl_econ = (car_period_runs_conceded / car_period_balls_bowled) * 6 if car_period_balls_bowled > 0 else np.nan # Handle division by zero
    ...
    if 'out_kind' in country_stats_df.columns:
        ...
        if not dismissals_df.empty:
            ...
            dismissals_df['out_kind'] = dismissals_df['out_kind'].fillna('Unknown')
            dismissal_counts = dismissals_df["out_kind"].value_counts().reset_index()
            dismissal_counts.columns = ["Dismissal Type", "Count"]
            # Exclude 'not out' if it sneaks in
            dismissal_counts = dismissal_counts[dismissal_counts["Dismissal Type"].str.lower() != 'not out']
            fig_dismissals = plot_dismissal_pie(dismissal_counts, title=f"Dismissals vs {selected_opposition}")
        else:
             fig_dismissals = go.Figure(layout=go.Layout(title=f"Dismissals vs {selected_opposition}<br>(No dismissals recorded in period)"))
    else:
        fig_dismissals = go.Figure(layout=go.Layout(title=f"Dismissals vs {selected_opposition}<br>(Dismissal data unavailable)"))
    ...
    fig_runs = plot_run_contribution_pie(runs_from_4s, runs_from_6s, other_runs, title=f"Run Scoring Breakdown vs {selected_opposition}")

    # Comparison Visuals (Opponent Period vs Career Period)
    fig_gauge_bat_avg = create_comparison_gauge(opp_bat_avg, car_period_bat_avg, "Batting Average", higher_is_better=True)
    fig_gauge_bowl_econ = create_comparison_gauge(opp_bowl_econ, car_period_bowl_econ, "Bowling Economy", upper_bound=15, higher_is_better=False) # Economy: lower is better (implicit in gauge threshold)
    fig_bar_bat_sr = create_comparison_bar(opp_bat_sr, car_period_bat_sr, f"vs {selected_opposition}", "Career (Period)", "Batting Strike Rate")
    ...
    fig_bar_wkts = create_comparison_bar(opp_wickets, car_period_wickets, f"vs {selected_opposition}", "Career (Period)", "Total Wickets Taken")
    ...
    stats_layout = html.Div([
        dbc.Card([
            dbc.CardHeader([
                ...
                html.P(subheader_text, className="text-center text-muted small mb-0")
            ]),
            dbc.CardBody([
                # Summary Row
                dbc.Row([
                    dbc.Col(dbc.Card(dbc.CardBody([
                        html.H6("Overall vs Opponent (Period)", className="card-title text-center"),
                        html.P(f"Matches: {opp_matches}"), # Use matches calculated earlier
                    ...
                    dbc.Col(dbc.Card(dbc.CardBody([
                        html.H6("Batting vs Opponent (Period)", className="card-title text-center"),
                        html.P(f"Average: {f'{opp_bat_avg:.2f}' if pd.notna(opp_bat_avg) else 'N/A'}"),
                        html.P(f"Strike Rate: {f'{opp_bat_sr:.2f}' if pd.notna(opp_bat_sr) else 'N/A'}"),
                        html.P(f"Outs: {opp_outs}")
                        ]), color="light", className="h-100"), width=6, md=4, className="mb-3"),
                    dbc.Col(dbc.Card(dbc.CardBody([
                        html.H6("Bowling vs Opponent (Period)", className="card-title text-center"),
                        html.P(f"Average: {f'{opp_bowl_avg:.2f}' if pd.notna(opp_bowl_avg) else 'N/A'}"),
                        html.P(f"Economy: {f'{opp_bowl_econ:.2f}' if pd.notna(opp_bowl_econ) else 'N/A'}"),
                        html.P(f"Wickets: {opp_wickets}")
                        ]), color="light", className="h-100"), width=6, md=4, className="mb-3"),
                ], className="mb-4"),
                html.Hr(),
                # Comparison Row
                html.H5("Comparison: Performance vs Opponent vs Career Average (within Period)", className="text-center mb-3"),
                dbc.Row([
                     dbc.Col(dcc.Graph(figure=fig_gauge_bat_avg, config={'displayModeBar': False}), width=12, sm=6, lg=3, className="mb-3"),
                     dbc.Col(dcc.Graph(figure=fig_bar_bat_sr, config={'displayModeBar': False}), width=12, sm=6, lg=3, className="mb-3"),
                     dbc.Col(dcc.Graph(figure=fig_gauge_bowl_econ, config={'displayModeBar': False}), width=12, sm=6, lg=3, className="mb-3"),
                     dbc.Col(dcc.Graph(figure=fig_bar_wkts, config={'displayModeBar': False}), width=12, sm=6, lg=3, className="mb-3"),
                ], className="mb-4 align-items-stretch"), # Use align-items-stretch if heights vary
                html.Hr(),
                 # Detailed Analysis Row
                html.H5(f"Detailed Analysis vs {selected_opposition} (Period)", className="text-center mb-3"),
                 dbc.Row([
                    dbc.Col(dcc.Graph(figure=fig_runs, config={'displayModeBar': False}), width=12, md=6, className="mb-3"),
                    dbc.Col(dcc.Graph(figure=fig_dismissals, config={'displayModeBar': False}), width=12, md=6, className="mb-3"),
                ]),
                # Dismissal Table (conditional display)
                dbc.Row([
                     dbc.Col([
                         html.H6("Dismissal Details (vs Opponent, Period)", className="mt-3 text-center"),
                         dash_table.DataTable(
                            data=dismissal_counts.to_dict('records'),
                            columns=[{'name': i, 'id': i} for i in dismissal_counts.columns],
                 ...
                 className="mt-3 justify-content-center", # Center the column containing the table
                 style={'display': 'flex' if not dismissal_counts.empty else 'none'} # Hide row if no dismissal data
                ),

                # Download Buttons
                html.Hr(className="my-4"),
                html.Div([
                     html.H5("Download Filtered Data (Period)", className="text-center mb-3"),
                     dbc.Row([
                         dbc.Col(dbc.Button( [html.I(className="fas fa-download me-2"), "Download CSV (vs Opponent, Period)"], id="analyzer-csv-button", n_clicks=0, color="primary"), width="auto", className="d-flex justify-content-center mb-2 mb-md-0"), # Add margin bottom for small screens
                         dbc.Col(dbc.Button( [html.I(className="fas fa-file-pdf me-2"), "Download PDF Summary (vs Opp., Period)"], id="analyzer-pdf-button", n_clicks=0, color="danger"), width="auto", className="d-flex justify-content-center"),
...
def analyzer_download_csv(n_clicks, player, format, opposition, start_date, end_date, stored_data):
    ...
    print(f"Callback triggered: analyzer_download_csv (Clicks: {n_clicks}, Period: {start_date}-{end_date})")
    df_full = deserialize_data(stored_data);
    if df_full is None or df_full.empty:
    ...
    try:
        ...
        safe_opposition = "".join(c if c.isalnum() else "_" for c in opposition)
        ...
        csv_string = country_stats_df_period.to_csv(index=False, encoding='utf-8')
        return dict(content=csv_string, filename=fn) # Use dict format for dcc.send_data_frame alternative
...
def analyzer_download_pdf(n_clicks, player, format, opposition, start_date, end_date, stored_data):
    ...
    print(f"Callback triggered: analyzer_download_pdf (Clicks: {n_clicks}, Period: {start_date}-{end_date})")
    df_full = deserialize_data(stored_data);
    if df_full is None or df_full.empty:
    ...
    try:
        ...
        batting_strike_rate = (total_runs / balls_faced) * 100 if balls_faced > 0 else 0.0
        ...
        total_runs_conceded = int(country_stats_df_period["runs_conceded"].sum())
        bowling_avg = total_runs_conceded / total_wickets if total_wickets > 0 else np.nan
        bowling_economy = (total_runs_conceded / balls_bowled) * 6 if balls_bowled > 0 else np.nan # Handle div by zero
        ...
        pdf_buffer = export_pdf(pdf_text) # Assumes export_pdf function is defined
