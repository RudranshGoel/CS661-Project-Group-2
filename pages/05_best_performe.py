...
def deserialize_data(stored_data):
    ...
    try:
        ...
        print(f"ERROR: Failed to deserialize data from store (performance analysis): {e}")
...
BOWLER_RANKING_STATS = {
    ...
    'bowling_average': {'label': 'Bowling Average', 'higher_is_better': False}, # Lower is better
    'economy': {'label': 'Economy Rate', 'higher_is_better': False}, # Lower is better
    'bowling_strike_rate': {'label': 'Bowling Strike Rate', 'higher_is_better': False}, # Lower is better
...
MIN_BALLS_BOWLED = 60       # Minimum balls bowled (e.g., 10 overs) for bowler ranking
    ...
    dbc.Card(
        dbc.CardBody([
            dbc.Row([
                ...
                dbc.Col([
                    html.Label("Filter by Player's Team:", className="fw-bold"),
                    dcc.Dropdown(
                        id='perf-country-dropdown', # Unique ID for this component
                        options=[{'label': 'Global (All Teams)', 'value': 'Global'}], # Initial option
                ...
                dbc.Col([
                    ...
                    dcc.Dropdown(
                        ...
                        options=[{'label': v['label'], 'value': k} for k, v in BATSMAN_RANKING_STATS.items()],
                        value=list(BATSMAN_RANKING_STATS.keys())[0], # Default value (first batsman stat)
                        clearable=False
                    )
                ], width=12, md=3, className="mb-3"),
                ...
                dbc.Col([
                    html.Label("Number of Players (N):", className="fw-bold", id='perf-slider-label'), # Unique ID for label
                    dcc.Slider(
                        id='perf-n-slider', # Unique ID
                        min=3, max=15, step=1, value=5, # Slider parameters
                        marks={i: str(i) for i in range(3, 16, 2)}, # Marks displayed on the slider
                        tooltip={"placement": "bottom", "always_visible": True} # Tooltip showing value
                    )
                ], width=12, md=3, className="mb-3"),
            ]),
            # Row to display text about the ranking thresholds being applied
            dbc.Row(dbc.Col(
                 html.Small(id='perf-threshold-info', className="text-muted fst-italic") # Text updated by callback
            ))
...
@callback(
    Output('perf-country-dropdown', 'options'), # Output: updates the dropdown options
    Input('main-data-store', 'data'),           # Input: triggered when data store changes
...
@callback(
    Output('perf-stat-dropdown', 'options'),      # Output 1: updates stat options
    Output('perf-stat-dropdown', 'value'),        # Output 2: updates default selected stat
    Output('perf-threshold-info', 'children'),   # Output 3: updates threshold info text
    Input('perf-analysis-type-selector', 'value') # Input: triggered by analysis type change
)
def update_stat_dropdown_options(analysis_type):
    ...
    print(f"Callback triggered: update_stat_dropdown_options (Type: {analysis_type})")
    if analysis_type == 'Batsman':
        ...
        options = [{'label': v['label'], 'value': k} for k, v in BOWLER_RANKING_STATS.items()]
    ...
    print(f" - Setting stat options for {analysis_type}, default: {default_value}")
...
@callback(
    ...
    Output('perf-slider-label', 'children'),# Output 3: Slider label text (showing actual N)
    Input('perf-country-dropdown', 'value'),        # Input 1: Selected Country/Global
    Input('perf-analysis-type-selector', 'value'),  # Input 2: Selected Analysis Type
    Input('perf-stat-dropdown', 'value'),           # Input 3: Selected Statistic
    Input('perf-n-slider', 'value'),                # Input 4: Selected N value
    Input('main-data-store', 'data')                # Input 5: Main data
)
def update_performance_charts(selected_country, analysis_type, selected_stat, n_players, stored_data):
    """
    ...
    """
    print(f"Callback triggered: update_performance_charts (Country: {selected_country}, Type: {analysis_type}, Stat: {selected_stat}, N: {n_players})")
    slider_label_text = f"Number of Players (N={n_players}):" # Default slider label
    ...
    if df_full is None or df_full.empty:
        ...
        fig_empty = go.Figure(layout=go.Layout(title="Data not loaded", title_x=0.5))
    ...
    if analysis_type == 'Batsman':
        required_cols = base_cols + ['runs_scored', 'balls_faced', 'player_out', 'fours_scored', 'sixes_scored']
        stat_config_dict = BATSMAN_RANKING_STATS
    elif analysis_type == 'Bowler':
        required_cols = base_cols + ['wickets_taken', 'runs_conceded', 'balls_bowled']
        ...
        fig_err = go.Figure(layout=go.Layout(title="Invalid Analysis Type Selected", title_x=0.5))
    ...
    if not all(col in df_full.columns for col in required_cols):
        ...
        print(f"  - Preventing update: Missing required columns for {analysis_type}: {missing}")
    ...
    try:
        ...
        if selected_country == 'Global':
            ...
            if selected_country not in df_full['player_team'].unique():
                 print(f"  - Selected country '{selected_country}' not found in data.")
                 fig_err = go.Figure(layout=go.Layout(title=f"Team '{selected_country}' not found", title_x=0.5))
                 return fig_err, fig_err, slider_label_text + " Error"
            # Filter the DataFrame for the selected team
            df_filtered = df_full[df_full['player_team'] == selected_country].copy()
        ...
        if df_filtered.empty:
            ...
            fig_empty = go.Figure(layout=go.Layout(title=f"No player data for {country_label}", title_x=0.5))
        ...
        qualified_players = pd.DataFrame() # Initialize DataFrame for players meeting thresholds

        # Aggregate and calculate metrics based on whether Batsman or Bowler is selected
        if analysis_type == 'Batsman':
            # Calculate innings and matches batted
            batted_in_match = df_filtered[df_filtered['balls_faced'] > 0].groupby(['name', 'match_id']).size().reset_index(name='batted_flag')
            innings_count = batted_in_match.groupby('name')['match_id'].nunique().reset_index(name='innings')
            matches_batted_count = batted_in_match.groupby('name')['match_id'].nunique().reset_index(name='matches_batted')
            ...
            player_stats = pd.merge(player_stats, innings_count, on='name', how='left')
            player_stats = pd.merge(player_stats, matches_batted_count, on='name', how='left')
            player_stats['innings'] = player_stats['innings'].fillna(0).astype(int)
            player_stats['matches_batted'] = player_stats['matches_batted'].fillna(0).astype(int)

            # Calculate derived batting metrics (average, strike rate)
            player_stats['average'] = player_stats.apply(lambda r: r['total_runs'] / r['outs'] if r['outs'] > 0 else np.nan, axis=1)
            player_stats['strike_rate'] = player_stats.apply(lambda r: (r['total_runs'] / r['balls_faced']) * 100 if r['balls_faced'] > 0 else 0.0, axis=1)
            ...
            bowled_in_match = df_filtered[df_filtered['balls_bowled'] > 0].groupby(['name', 'match_id']).size().reset_index(name='bowled_flag')
            ...
            player_stats = pd.merge(player_stats, matches_bowled_count, on='name', how='left')
            player_stats['matches_bowled'] = player_stats['matches_bowled'].fillna(0).astype(int)

            # Calculate derived bowling metrics (average, economy, strike rate)
            player_stats['bowling_average'] = player_stats.apply(lambda r: r['runs_conceded'] / r['wickets'] if r['wickets'] > 0 else np.nan, axis=1)
            player_stats['economy'] = player_stats.apply(lambda r: (r['runs_conceded'] / r['balls_bowled']) * 6 if r['balls_bowled'] > 0 else np.nan, axis=1)
            player_stats['bowling_strike_rate'] = player_stats.apply(lambda r: r['balls_bowled'] / r['wickets'] if r['wickets'] > 0 else np.nan, axis=1)
        ...
        if qualified_players.empty:
            print(f"  - No players qualify for ranking in {country_label} for {analysis_type} based on thresholds.")
            fig_empty = go.Figure(layout=go.Layout(title=f"No players qualify for ranking ({analysis_type})", title_x=0.5))
            return fig_empty, fig_empty, slider_label_text
        ...
        if selected_stat not in stat_config_dict or selected_stat not in qualified_players.columns:
             print(f"  - Invalid or missing ranking statistic: {selected_stat} for {analysis_type}")
             fig_err = go.Figure(layout=go.Layout(title=f"Invalid statistic selected: {selected_stat}", title_x=0.5))
             return fig_err, fig_err, slider_label_text + " Error"
        ...
        if qualified_players.empty:
            print(f"  - No players with valid '{stat_label}' data after dropping NaNs.")
            fig_empty = go.Figure(layout=go.Layout(title=f"No valid '{stat_label}' data for ranking", title_x=0.5))
            return fig_empty, fig_empty, slider_label_text

        # --- 4. Sort and Select Top/Bottom N ---
        # Sort players based on the ranking column (always ascending initially)
        qualified_players_sorted = qualified_players.sort_values(by=ranking_col, ascending=True, na_position='last')
        ...
        slider_label_text = f"Number of Players (N={n_players}, Showing {actual_n}):" # Update slider label
        ...
        def format_stat_value(value, stat_key, analysis_type):
            ...
            decimal_stats_bowler = ['bowling_average', 'economy', 'bowling_strike_rate']
            ...
            if is_decimal: return f"{value:.2f}" # Format as float with 2 decimals
            else:
                try: return f"{int(value):,}" # Format as integer with commas
                except (ValueError, TypeError): return f"{value:.2f}" # Fallback formatting
        ...
        if not top_n.empty:
            ...
            if higher_is_better:
                best_player_row = top_n.loc[top_n[ranking_col].idxmax()] # Find row with max value
            else:
                best_player_row = top_n.loc[top_n[ranking_col].idxmin()] # Find row with min value
            best_name = best_player_row['name']             # Get best player's name
            best_value = best_player_row[ranking_col]       # Get best player's stat value
            formatted_best_value = format_stat_value(best_value, selected_stat, analysis_type) # Format value
            ...
            fig_top.add_annotation(
                text=f"<b>Best:</b><br>{best_name}<br>{formatted_best_value}", # Text content
            ...
            fig_top = go.Figure(layout=go.Layout(title=f"No Top {actual_n} {analysis_type} Data Available", title_x=0.5))
        ...
        if not bottom_n.empty:
            ...
            worst_name = worst_player_row['name']           # Get worst player's name
            worst_value = worst_player_row[ranking_col]     # Get worst player's stat value
            formatted_worst_value = format_stat_value(worst_value, selected_stat, analysis_type) # Format value
            ...
            fig_bottom.add_annotation(
             ...
             fig_bottom = go.Figure(layout=go.Layout(title=f"No Bottom {actual_n} {analysis_type} Data Available", title_x=0.5))
        ...
        for fig in [fig_top, fig_bottom]:
             if fig.data: # Apply only if the figure has data/annotations
                 fig.update_layout(showlegend=False, margin=dict(t=60, b=20, l=20, r=20), title_x=0.5)

        print(f"  - Charts generated successfully for Top/Bottom {actual_n} {analysis_type}s.")
