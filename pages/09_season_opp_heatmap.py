...
dash.register_page(__name__, name='Season Heatmap', path='/heatmap', title='Season × Opponent Heatmap')
...
layout = dbc.Container([
    dbc.Row(dbc.Col(html.H2("Performance Heatmap: Season × Opponent"), width=12, className="my-4")),
    ...
    dbc.Row([
        dbc.Col(dcc.Loading(dcc.Graph(id='heatmap-boxplot'), type='circle'), width=12)
    ], className="mt-4")
], fluid=True)
...
@callback(
    ...
    Input('heatmap-format-radio','value'),
    Input('heatmap-metric-dropdown','value'),
    State('main-data-store','data')
)
def update_heatmap(player, fmt, metric, store_data):
    df = deserialize_data(store_data)
    ...
    d = df[(df['name']==player) & (df['match_type']==fmt) & df['start_date'].notna()].copy()
    d['season'] = d['start_date'].dt.year

    # Compute chosen metric per season × opponent
    if metric == 'bat_avg':
        agg = d.groupby(['season','opposition_team']).apply(
            lambda g: g['runs_scored'].sum() / g['player_out'].sum() if g['player_out'].sum()>0 else np.nan
        ).reset_index(name='value')
        color_label = 'Batting Average'
    elif metric == 'strike_rate':
        agg = d.groupby(['season','opposition_team']).apply(
            lambda g: (g['runs_scored'].sum() / g['balls_faced'].sum() * 100) if g['balls_faced'].sum()>0 else np.nan
        ).reset_index(name='value')
        color_label = 'Strike Rate'
    else:  # total_runs
        agg = d.groupby(['season','opposition_team'])['runs_scored'].sum().reset_index(name='value')
...
def update_boxplot(clickData, player, fmt, metric, store_data):
    ...
    if metric == 'bat_avg':
        d['metric'] = d.apply(lambda r: r['runs_scored']/r['player_out'] if r['player_out']>0 else np.nan, axis=1)
        ylabel = 'Batting Average per Match'
    elif metric == 'strike_rate':
        d['metric'] = d.apply(lambda r: (r['runs_scored']/r['balls_faced']*100) if r['balls_faced']>0 else np.nan, axis=1)
